...
function M._stream(opts)
  -- Cancellation state for this stream.
  -- - opts.session_ctx isolates each avante instance so that cancelling one
  --   stream does not affect concurrent streams in other sidebar instances.
  -- - session_ctx.is_cancelled is the legacy per-session flag, observed by the
  --   tool runner's polling loop. It is set later (in the on_stop "cancelled"
  --   branch) to ask any still-running tools to abort.
  -- - session_ctx.cancel_token (allocated just below) is the preferred richer
  --   per-request handle. Soft-cancels via the token leave in-flight tools
  --   running and route their results through on_orphaned_result so the next
  --   outbound payload stays structurally consistent.
  opts.session_ctx = opts.session_ctx or {}
  opts.session_ctx.is_cancelled = false
  -- Keep the legacy module-global in sync for any code that still reads it
  -- directly. New code should use the per-request cancel_token instead.
